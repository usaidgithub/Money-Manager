...
const pool = mysql.createPool({
    ...
    database: process.env.DB_NAME,
    port: process.env.DB_PORT,
    waitForConnections: true,  // Wait if all pools are busy
    connectionLimit: 10,       // Max pools in the pool
...
app.post('/register_user', async (req, res) => {
    ...
    pool.query(sql, [username, hashedPassword, email], (err, results) => {
        if (err) throw err;
...
app.post('/add_income',(req,res)=>{
    ...
    pool.query(sql,[userId,amount,expenses_category,note,month,year],(err,results)=>{
        if(err) throw err
...
app.get('/fetch_income',(req,res)=>{
    ...
    pool.query(sql,[userId,month,year],(err,results)=>{
        if (err) {
...
app.delete('/delete_income/:id',(req,res)=>{
    ...
    pool.query(sql,[userId,recordid],(err,results)=>{
        if(err){
...
app.post('/update_income',(req,res)=>{
    ...
    pool.query(sql,[update_value,userId,id],(err,results)=>{
        if(err){
...
app.post('/update_expenses',(req,res)=>{
    ...
    pool.query(sql,[update_value,userId,id1],(err,results)=>{
        if(err){
...
app.post('/add_budget',(req,res)=>{
    ...
    pool.query(sql,[userId,expenses_category,amount,month,year],(err,results)=>{
        if(err) throw err
...
app.post('/update_budget',(req,res)=>{
    ...
    pool.query(sql,[update_value,userId,id2],(err,results)=>{
        if(err){
...
app.get('/fetch_yearincome',(req,res)=>{
    ...
    pool.query(sql,[userId,year],(err,results)=>{
        if (err) {
            console.error("Error fetching income data", err);
            res.status(500).send("Server error");
            return;
